<?php
// Week 3: Establish connection and read live data from tables
include_once('db_connect.php');

$storeName = "QuickMart Mini-Supermarket";

// Fetch items dynamically from the MySQL database table (newest first)
$sql = "SELECT * FROM products ORDER BY id DESC";
$result = mysqli_query($conn, $sql);

// Initialize an array to catch database items
$products = [];
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    // Fallback notice display if your database table has no products yet
    $products = [
        ["name" => "Database Setup Complete! Please add items via the Admin Panel.", "category" => "SYSTEM STATUS", "price" => 0.00, "image" => "⚙️"]
    ];
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <div style="background-color: #28a745; color: white; padding: 12px 30px; text-align: center; font-size: 14px; font-weight: 600; border-bottom: 2px solid #1e7e34;">
        ✓ Live Database Dynamic Catalog Stream Active (`products` table synced).
    </div>
    ...
    <header>
        ...
        <div class="nav-links">
            <a href="admin.php" style="color: #e67e22; margin-right: 15px; font-weight: bold;">Go to Admin Panel →</a>
            <a href="#">Aisles</a>
            <a href="#">My Cart (0)</a>
        </div>
    </header>
    ...
    <div class="main-container">
        ...
        <main class="catalog">
            <h2>Current Stock Availability (Live MySQL Stream)</h2>
